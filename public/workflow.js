...
function initThumbnailEditor() {
  ...
  function resizeHandle(layer, point) {
    ...
    const nearX = Math.abs(point.x - (layer.x + (point.x < layer.x ? -layer.width / 2 : layer.width / 2))) <= 32;
    const nearY = Math.abs(point.y - (layer.y + (point.y < layer.y ? -layer.height / 2 : layer.height / 2))) <= 32;
    if (!nearX || !nearY) return false;
    return (point.x < layer.x ? 'w' : 'e') + (point.y < layer.y ? 'n' : 's');
  }
  ...
  canvas.addEventListener('pointermove', event => { if (!editor.drag) return; const point = pointFromEvent(event); const dx = point.x - editor.drag.point.x; const dy = point.y - editor.drag.point.y; const totalDx = point.x - editor.drag.startPoint.x; const totalDy = point.y - editor.drag.startPoint.y; if (Math.abs(dx) + Math.abs(dy) > 2) editor.drag.moved = true; if (editor.drag.type === 'resize') { const layer = editor.drag.layer; const fromLeft = editor.drag.handle.includes('w'); const fromTop = editor.drag.handle.includes('n'); layer.width = Math.max(20, editor.drag.startWidth + (fromLeft ? -totalDx : totalDx)); layer.height = Math.max(20, editor.drag.startHeight + (fromTop ? -totalDy : totalDy)); const widthDelta = layer.width - editor.drag.startWidth; const heightDelta = layer.height - editor.drag.startHeight; layer.x = editor.drag.startX + (fromLeft ? -widthDelta / 2 : widthDelta / 2); layer.y = editor.drag.startY + (fromTop ? -heightDelta / 2 : heightDelta / 2); } else { editor.drag.layer.x += dx; editor.drag.layer.y += dy; } editor.drag.point = point; renderCanvas(); });
